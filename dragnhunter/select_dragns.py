...
def add_source_flag(data, bad_filter, goodval=0, badval=1,
                    colname='Source_flag', flag_dtype=int,
                    description='Source quality flag (>0 is suspect)'):
    'add flag to sources'
    flag = goodval*np.ones(len(data)).astype(flag_dtype)
    flag[bad_filter] = badval
    data[colname] = flag
    data[colname].description = description
...
#    components = Table.read(component_file)
    ...
    dragns = add_source_flag(data=dragns, bad_filter=dflag_filt,
                             colname='Source_flag',
                             description='Source quality flag (>0 is suspect)')
    single_comps['Source_flag'] = 0 ##may need to change this in future versions
    ###add in description
    single_comps['Source_flag'].description = 'Source quality flag (>0 is suspect)'
    
    ###make source list
    dragncols = ['Name', p_ra, p_dec, 'Flux', 'E_Flux', 'LAS',
                 'E_LAS', 'Source_flag']
    sccols = [cname, c_ra, c_dec, cflux, cfluxerr, cmaj, emaj, 'Source_flag']
    ...
    ###add in host finding
    if find_hosts==True:
        host_data = find_allwise(source_list, racol=p_ra, decol=p_dec,
                                 maxsep=search_rad, timeout=htimeout)
